...
function buildBrandGrowthStrategyPrompt() {
  return [
    ...
    "每条趋势必须按以下判断框架写成策略结论（自然融入 summary/reason/ideas，不要输出小标题清单）：",
    "过去：消费者原先怎么想、怎么做？",
    "现在：消费者发生了什么具体变化？",
    "原因：为什么发生（必须落到可观察的内容形式、场景或矛盾，不得空喊升级）？",
    "品牌：为什么与本品牌相关（强化优势 / 创造新消费场景 / 避开竞品红海）？",
    "动作：应该生产什么内容（ideas 必须可直接派发执行）？",
...
function buildTrendAnalysisSystemPrompt(bucketMeta = [TREND_BUCKET_META[0]], options = {}) {
  ...
  return [
    ...
    "你要根据完整品牌档案、品牌智能层结论和当前 bucket 证据，输出品牌增长机会趋势与选题骨架；完整发布文案与视觉资产会在用户实际生成内容时另行创建。",
    ...
    "每条 trend 必须包含：id 或 stableKey、title, category, summary, score, novelty_score, brand_fit_score, actionability_score, tags, reason, ideas。",
    "trend.title 控制在 16-42 个中文字符，summary 控制在 45-90 个中文字符，reason 控制在 40-110 个中文字符；summary/reason 要覆盖策略框架中的过去→现在→原因→品牌，ideas 承接动作；说清后立即结束，不写重复结论。",
...
function buildTrendAnalysisUserPrompt(brand, options = {}, bucketMeta = [TREND_BUCKET_META[0]]) {
  ...
  return [
    ...
    "6. 选题要能直接给运营同学使用，标题、角度、钩子要有小红书笔记感，避免空泛文案。",
...
function normalizeTrendSet(rawTrends, brand, baseId, options = {}) {
  ...
  return source
    ...
    .map((trend, index) => {
      ...
      const score = normalizeModelScore(trend?.score);
      ...
      return {
        ...
        reason: String(trend?.reason || "").trim(),
...
function formatTrendRetryFeedback(issues) {
  ...
  if (["missing-trend-field", "insufficient-reason-detail", "invalid-score", "invalid-self-score", "missing-trend-tags", "idea-count", "missing-idea-field", "missing-idea-tags"].some((reason) => reasons.has(reason))) {
    feedback.push(`补齐每条趋势和两条 idea 的全部 schema 字段（含 novelty_score、brand_fit_score、actionability_score）；reason 至少用 36 个中文字符说明具体来源话题、内容机会和判断边界，趋势与 idea 的 tags 都要有 3-5 个，三项自评分均 ≥ ${TREND_SELF_SCORE_MIN}。 `);
  }
...
function buildTargetedTrendRepairSystemPrompt(bucketMeta, repairCount) {
  ...
  return [
    ...
    "每条 item 必须包含：stableKey、title、category、summary、score、novelty_score、brand_fit_score、actionability_score、tags、reason、evidenceIds、ideas。",
...
function getBrandClaimTextEntries(trend) {
  const entries = ["title", "category", "summary", "reason"].map((field) => ({
    field,
      ...
      if (!validationIssues.length) {
        ...
        if (!postAliasIssues.length) {
          trendBuckets = finalizeModelTrendBuckets(
            attachEvidenceSnapshots(userVisibleBuckets, anySearchEvidence, pgyEvidence),
            selectedBucketMeta,
            baseId,
          );
          break;
