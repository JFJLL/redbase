...
function loadAppConfig() {
  ...
  return {
    video: {
      ...
      pollMaxBackoffMs: Math.max(2000, Number(process.env.VIDEO_POLL_MAX_BACKOFF_MS || merged.video?.pollMaxBackoffMs || 30000)),
      submitTimeoutMs: Math.max(1000, Number(process.env.VIDEO_SUBMIT_TIMEOUT_MS || merged.video?.submitTimeoutMs || 45000)),
      pollTimeoutMs: Math.max(1000, Number(process.env.VIDEO_POLL_TIMEOUT_MS || merged.video?.pollTimeoutMs || 20000)),
      schedulerIntervalMs: Math.max(100, Number(process.env.VIDEO_SCHEDULER_INTERVAL_MS || merged.video?.schedulerIntervalMs || 500)),
      maxClipBytes: Math.max(1024 * 1024, Number(process.env.VIDEO_MAX_CLIP_BYTES || merged.video?.maxClipBytes || 256 * 1024 * 1024)),
      maxFinalBytes: Math.max(1024 * 1024, Number(process.env.VIDEO_MAX_FINAL_BYTES || merged.video?.maxFinalBytes || 1024 * 1024 * 1024)),
      remoteTimeoutMs: Math.max(1000, Number(process.env.VIDEO_REMOTE_TIMEOUT_MS || merged.video?.remoteTimeoutMs || 30000)),
      d2MaxConcurrentSubmissions: Math.max(1, Number(process.env.VIDEO_D2_MAX_CONCURRENT_SUBMISSIONS || merged.video?.d2MaxConcurrentSubmissions || 4)),
      mediaMaxConcurrency: Math.max(1, Number(process.env.VIDEO_MEDIA_MAX_CONCURRENCY || merged.video?.mediaMaxConcurrency || 3)),
      ffmpegMaxConcurrency: Math.max(1, Number(process.env.VIDEO_FFMPEG_MAX_CONCURRENCY || merged.video?.ffmpegMaxConcurrency || 1)),
      runninghub: {
