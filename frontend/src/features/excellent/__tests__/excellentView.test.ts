...
describe("ExcellentView", () => {
  ...
  it("proxies remote card and carousel images through the same-origin image endpoint", async () => {
    const remoteItems = [
  ...
  it("uses xingtu filters, neutral detail copy and metadata learning when no transcript is available", async () => {
    ...
    await flushPromises();
    expect(wrapper.find(".xingtu-video-player").attributes("src")).toBe("/api/xingtu/videos/7675709137612013818/media");
